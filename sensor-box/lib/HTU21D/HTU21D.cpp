...
int HTU21D::sample_ctemp(void) {
    ...
    Thread::wait(50); // Per datasheet, wait long enough for device to sample temperature
...
int HTU21D::sample_humid(void) {
    ...
    i2c_->write((HTU21D_I2C_ADDRESS << 1) & 0xFE, tx, 1);
    Thread::wait(16); // Per datasheet, wait long enough for device to sample humidity
